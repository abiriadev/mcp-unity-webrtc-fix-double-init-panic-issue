...
public class SampleBehaviour : MonoBehaviour
{
    ...
    private class ConferenceDelegate : IConferenceDelegate
    {
        ...
        public IEnumerator VideoReceived(Participant participant, VideoStreamTrack videoTrack, Texture texture)
        {
            ...
            this.behaviour.AddRemoteTile(videoTrack.Id, texture);
            yield return null;
        }
    ...
    IEnumerator JoinConference()
    {
        ...
        // this.connection = new Connection("wss://abiria-lt-arch.local/xmpp-websocket", "meet.jitsi", false);
        this.connection = new Connection("wss://meet.jit.si/xmpp-websocket", "meet.jit.si", false);

        Debug.Log("Joining conference");
        // join() currently blocks until the room is joined.
        // This will be changed to be asynchronous in future
        this.conference = this.connection.join("aipim", "nick1", localTracks, new ConferenceDelegate(this));

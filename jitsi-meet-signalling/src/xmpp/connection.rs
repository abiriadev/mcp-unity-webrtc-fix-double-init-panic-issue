...
use format_xml::format_xml_with_indent_unwrap;
use futures::{
  sink::{Sink, SinkExt},
...
impl Connection {
    ...
    S::Error: std::error::Error + Send + Sync + 'static,
  {
    ...
    while let Some(element) = rx.next().await {
      ...
      debug!(
        "XMPP    {}\n{}",
        ">>> OUTGOING STANZA".if_supports_color(OwoStream::Stdout, |x| x.green()),
        format_xml_with_indent_unwrap(&xml, "    ")
      );
    ...
    loop {
      ...
      let element: Element = match message {
        Message::Text(xml) => {
          debug!(
            "XMPP    {}\n{}",
            "<<< INCOMING STANZA".if_supports_color(OwoStream::Stdout, |x| x.red()),
            format_xml_with_indent_unwrap(&xml, "    ")
          );
          xml.parse()?
        },
